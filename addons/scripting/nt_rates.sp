...
void RestoreRate(client, rateType)
{
	...
	switch (rateType)
	{
		case TYPE_RATE:
		{
			...
			PrintToChatAll("%s Player %s had an invalid rate. Value has been reset to %s", g_tag, clientName, defaultRate);
		}
		...
		case TYPE_CMDRATE:
		{
			...
			PrintToChatAll("%s Player \"%s\" had an invalid cl_cmdrate. Value has been reset to %s", g_tag, clientName, defaultCmdRate);
		}
		...
		case TYPE_UPDATERATE:
		{
			...
			PrintToChatAll("%s Player \"%s\" had an invalid cl_updaterate. Value has been reset to %s", g_tag, clientName, defaultUpdateRate);
		}
		...
		case TYPE_INTERP:
		{
			...
			PrintToChatAll("%s Player \"%s\" had an invalid cl_interp. Value has been reset to %s", g_tag, clientName, defaultInterp);
		}
		
		case TYPE_INTERP_ENABLED:
		{
			ClientCommand(client, "cl_interpolate 1");
			PrintToChatAll("%s Player \"%s\" had interpolation disabled. This has been reverted.", g_tag, clientName);
		}
	}
}
...
void CapInterp(client, capType)
{
	...
	switch (capType)
	{
		case TYPE_MIN:
		{
			...
			PrintToChatAll("%s Player \"%s\" had smaller cl_interp than allowed. Value has been capped to the minimum %s", g_tag, clientName, minInterp);
		}
		...
		case TYPE_MAX:
		{
			...
			PrintToChatAll("%s Player \"%s\" had bigger cl_interp than allowed. Value has been capped to the maximum %s", g_tag, clientName, maxInterp);
		}
	}
}